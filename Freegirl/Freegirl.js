AR.log("Freegirl AR v1.0");

AR.onload = function() {
      AR.audio_play("bundle/bg.mp3");
      AR.setInterval(function(){
        AR.audio_play("bundle/bg.mp3");
      },38000);


    var tmp_Video="bundle/"+Math.round(Math.random())+".mp4";
    AR.log(tmp_Video);
    var options = {};
    options.transparent = true;
    AR.video.set("Video",tmp_Video,1, JSON.stringify(options));
    AR.video.play("Video");
};
...
AR.onclick = function(nodeId, x, y) {
    if(nodeId=="UI_click"){
        ...
        antHelper.getTicket("freegirl");
        AR.audio_stop("bundle/bg.mp3");
    }
};

AR.onevent = function(eventName,extra){
    if(eventName=='playEnd'||extra=="playEnd"){
        AR.play("UI_click#Default",0);
        AR.set_visible("UI_click",true);
        AR.video.stop("Video");
    }
}
